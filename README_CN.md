...
PlayForge 是一个 AI 驱动的游戏设计平台，基于 Spring Boot 和 React 构建。通过 LangChain4J 集成多个大语言模型（OpenAI、Anthropic Claude、Google Gemini），帮助用户通过实时流式 AI 对话，头脑风暴、迭代打磨并产出专业级游戏策划文档。

## 功能特性

- **AI 对话** — 通过 WebSocket 与多个大模型进行实时流式对话
- **多模型支持** — 每次对话可选择 OpenAI GPT、Anthropic Claude 或 Google Gemini
- **用户认证** — 注册、登录、登出，JWT 双令牌（access/refresh）+ Redis 会话管理
- **个人资料管理** — 编辑昵称、简介、头像（直传阿里云 OSS）
...
├── common/           # 公共工具类、常量、异常
├── domain/           # 实体、值对象、仓储接口
├── infrastructure/   # 仓储实现、MyBatis、Redis、JWT、OSS、LLM 供应商
├── application/      # 应用服务、用例编排、AI 智能体工厂
├── api/              # REST 控制器、请求/响应 DTO、拦截器、WebSocket 处理器
...
| 方法 | 路径 | 说明 | 需认证 |
|------|------|------|--------|
| GET | `/api/agents` | 列出当前用户的智能体 | 是 |
| GET | `/api/agents/{id}` | 获取智能体详情 | 是 |
| POST | `/api/agents` | 创建智能体定义 | 是（管理员） |
| POST | `/api/agents/with-thread` | 创建智能体 + 会话线程 | 是（管理员） |
| DELETE | `/api/agents/{id}` | 删除智能体（软删除） | 是（管理员） |
| POST | `/api/agents/skills` | 创建技能 | 是 |
